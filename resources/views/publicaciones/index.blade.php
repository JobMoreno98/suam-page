...
@section('content')
    <div class="bg-gray-50/50 py-10 px-4 sm:px-6 lg:px-8">
        ...
        <div class="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8">
            ...
            <aside class="lg:w-64 flex-shrink-0 lg:sticky lg:top-8 lg:self-start mb-6 lg:mb-0 relative z-20"
                x-data="{ open: false }">
                <div class="bg-white rounded-3xl p-4 lg:p-6 border border-gray-100 shadow-sm">

                    {{-- BOTÓN TOGGLE (Solo visible en Móvil) --}}
                    <button @click="open = !open"
                        class="flex justify-between items-center w-full lg:hidden text-navy font-bold">
                        <span>{{ request('year') ? 'Año: ' . request('year') : 'Filtrar por Año' }}</span>
                        <svg class="w-5 h-5 transform transition-transform" :class="open ? 'rotate-180' : ''" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>

                    {{-- TÍTULO DESKTOP (Solo visible en Desktop) --}}
                    <h2 class="text-xl font-black text-navy mb-4 tracking-tight hidden lg:block">
                        Filtrar por Año
                    </h2>

                    {{-- ========================================== --}}
                    {{-- 1. MENÚ MÓVIL (Con animación de Alpine) --}}
                    {{-- ========================================== --}}
                    <ul x-show="open" x-collapse.duration.300ms class="space-y-2 mt-4 lg:hidden" x-cloak>
                        <li>
                            <a href="{{ route('publicaciones.index') }}"
                                class="block px-5 py-2.5 rounded-full text-sm font-medium transition-colors border shadow-sm
                        {{ !request('year') ? 'bg-navy text-white border-navy' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50' }}">
                                Todos
                            </a>
                        </li>
                        @foreach ($anios as $anio)
                            <li>
                                <a href="{{ route('publicaciones.index', ['year' => $anio]) }}"
                                    class="block px-5 py-2.5 rounded-full text-sm font-medium transition-colors border shadow-sm
                                {{ request('year') == $anio ? 'bg-navy text-white border-navy' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50' }}">
                                    {{ $anio }}
                                </a>
                            </li>
                        @endforeach
                        ...
                        <div
                            ...
                            <div class="p-6 flex flex-col flex-grow space-y-3">
                                ...
                                <div class="flex items-center gap-2 text-[11px] text-gray-400 font-medium">
                                    ...
                                    <span>{{ ceil(str_word_count(strip_tags($publicacion->contenido)) / 200) }} min de
                                        lectura</span>
                                </div>
