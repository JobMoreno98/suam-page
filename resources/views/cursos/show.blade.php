            ...
            <div class="flex items-center justify-between">
                ...
                @if ($curso->areaFormacion)
                    <a href="{{ route('areas.show', $curso->areaFormacion) }}"
                        class="inline-block text-xs font-bold border rounded-full px-3.5 py-1 bg-white text-brandgreen border-brandgreen/30 hover:bg-brandgreen hover:text-white transition-colors shadow-sm">
                ...
                <div class="lg:col-span-8 bg-white rounded-3xl p-6 sm:p-10 border border-gray-100 shadow-sm space-y-8">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 bg-gray-50 p-5 rounded-2xl border border-gray-100">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <span
                                    class="text-[10px] sm:text-xs text-gray-400 font-medium block uppercase tracking-wider">Duración</span>
                                <span
                                    class="text-xs sm:text-sm font-bold text-navy">{{ $curso->duracion ?? 'Por definir' }}</span>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <span
                                    class="text-[10px] sm:text-xs text-gray-400 font-medium block uppercase tracking-wider">Modalidad</span>
                                <span
                                    class="text-xs sm:text-sm font-bold text-navy">{{ $curso->modalidad ?? 'Presencial' }}</span>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-3 col-span-2 sm:col-span-1">
                            ...
                            <div>
                                <span
                                    class="text-[10px] sm:text-xs text-gray-400 font-medium block uppercase tracking-wider">Horario</span>
                                <span
                                    class="text-xs sm:text-sm font-bold text-navy">{{ $curso->horario ?? 'Consultar' }}</span>
                            </div>
                        </div>
                ...
                <div class="lg:col-span-4 space-y-6 lg:sticky lg:top-8">
                    ...
                    <div class="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm space-y-6">
                        ...
                        <div class="border-b border-gray-100 pb-4 text-center">
                            <span class="text-xs text-gray-400 font-bold uppercase tracking-wider block">Costo</span>
                            <div class="text-3xl font-black text-navy mt-1">
                                @if ($curso->precio && $curso->precio > 0)
                                    ${{ number_format($curso->precio, 2) }} <span
                                        class="text-xs font-normal text-gray-400">MXN</span>
                                @else
                        ...
                        @else
                            ...
                            <button type="button" disabled
                                class="w-full py-3 px-6 bg-gray-100 text-gray-400 font-bold text-sm rounded-xl cursor-not-allowed border border-gray-200 flex items-center justify-center gap-2">
                                <span>Sin inscripciones abiertas por el momento</span>
                            </button>
                        @endif

                        {{-- Info adicional rápida --}}
                        <ul class="text-xs text-gray-500 space-y-2.5 pt-2">
