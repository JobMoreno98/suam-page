...
@section('content')
    <div x-data="{
        ...
        // Paleta de colores intercalados para los acordeones
        bgColors: [
    ...
    class="bg-gray-50/50 py-10 px-4 sm:px-6 lg:px-8">
        ...
        <div class="max-w-7xl mx-auto space-y-8">
            ...
            {{-- ACORDEÓN PRINCIPAL: AGRUPADO POR CICLO --}}
            <div class="space-y-4">
                {{-- Asegúrate de enviar la variable $gruposPorCiclo desde tu controlador --}}
                @forelse($gruposPorCiclo as $ciclo => $grupos)
                    @php
                    ...
                    <div class="rounded-2xl border border-gray-200/80 overflow-hidden transition-all duration-300 shadow-sm bg-white"
                        :class="openCiclo === '{{ $ciclo }}' ? 'ring-2 ring-navy/10 shadow-md' : ''">

                        {{-- CABECERA DEL ACORDEÓN (CICLO) --}}
                        <button type="button"
                            @click="openCiclo = (openCiclo === '{{ $ciclo }}' ? null : '{{ $ciclo }}')"
                            ...
                            <div class="flex items-center gap-4 sm:gap-5 min-w-0">
                                ...
                                <div>
                                    <h2 class="text-lg sm:text-xl font-extrabold text-navy truncate">
                                        {{ $ciclo ? 'Ciclo ' . $ciclo : 'General / Sin Ciclo' }}
                                    </h2>
                            ...
                            <div class="text-navy p-1">
                                <svg class="w-6 h-6 transform transition-transform duration-300"
                                    :class="openCiclo === '{{ $ciclo }}' ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7" />
                                </svg>
                            </div>
                        </button>

                        {{-- CONTENIDO DESPLEGABLE DEL CICLO --}}
                        <div x-show="openCiclo === '{{ $ciclo }}'" x-cloak x-collapse class="bg-white border-t border-gray-100">
                ...
                @empty
                    <div class="bg-white rounded-3xl p-12 text-center border border-gray-200 shadow-sm">
                        <p class="text-gray-500 font-medium">Este curso no cuenta con materiales asignados a ningún ciclo actualmente.</p>
                    </div>
                @endforelse
            </div>
