<header x-data="{ 
    ...
    <div class="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 py-3.5 gap-4">
        ...
        <div class="flex items-center gap-3 min-w-0">
            {{-- Logo UdeG --}}
            <div class="text-center leading-tight hidden sm:block shrink-0 group cursor-default">
                <div class="text-[9px] font-black tracking-widest text-red-700 uppercase">Universidad de</div>
                <div class="text-[9px] font-black tracking-widest text-red-700 uppercase">Guadalajara</div>
            </div>
            ...
            <a href="{{ url('/') }}" class="flex items-center gap-2.5 group min-w-0">
                <div class="p-1 rounded-lg group-hover:bg-gray-50 transition-colors shrink-0">
                    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" class="transform group-hover:scale-105 transition-transform duration-200">
                        <rect x="2" y="12" width="5" height="10" rx="2" fill="#5fa93f"/>
                        <rect x="9" y="6" width="5" height="16" rx="2" fill="#2f7fb8"/>
                        <rect x="16" y="2" width="5" height="20" rx="2" fill="#f3a53a"/>
                    </svg>
                </div>
                <div class="min-w-0">
                    <div class="text-2xl font-black tracking-tight text-navy leading-none flex items-center gap-1">
                        sUAM
                        <span class="inline-block w-1.5 h-1.5 rounded-full bg-brandgreen"></span>
                    </div>
                    <div class="text-[8.5px] font-bold text-gray-400 mt-0.5 tracking-wider uppercase hidden lg:block truncate max-w-[260px]">
                        Sistema Universitario de Aprendizaje para Adultos Mayores
                    </div>
                </div>
            </a>
        </div>
        ...
        <div class="flex items-center gap-2 shrink-0">
            ...
            <form action="" method="GET" class="relative hidden sm:block">
                <input type="text" 
                       name="q" 
                       placeholder="Buscar cursos, sedes..." 
            ...
            <div class="flex items-center gap-1 lg:gap-2 text-sm text-white/80 font-medium">
                ...
                <a href="{{ route('home') }}" 
                   class="relative py-3.5 px-3.5 transition-colors duration-200 hover:text-white flex items-center gap-1.5 group {{ request()->routeIs('home') ? 'text-white font-semibold' : '' }}">
                    <span>Inicio</span>
                    <span class="absolute bottom-0 left-0 w-full h-0.5 bg-brandgreen transition-transform duration-200 transform scale-x-0 group-hover:scale-x-100 {{ request()->routeIs('home') ? 'scale-x-100' : '' }}"></span>
                </a>
                ...
                <a href="" 
                   class="relative py-3.5 px-3.5 transition-colors duration-200 hover:text-white flex items-center gap-1.5 group {{ request()->routeIs('recursos.*') ? 'text-white font-semibold' : '' }}">
                    <span>Recursos</span>
                    <span class="absolute bottom-0 left-0 w-full h-0.5 bg-brandgreen transition-transform duration-200 transform scale-x-0 group-hover:scale-x-100 {{ request()->routeIs('recursos.*') ? 'scale-x-100' : '' }}"></span>
                </a>
                ...
                <a href="" 
                   class="relative py-3.5 px-3.5 transition-colors duration-200 hover:text-white flex items-center gap-1.5 group {{ request()->routeIs('contacto') ? 'text-white font-semibold' : '' }}">
                    <span>Contacto</span>
                    <span class="absolute bottom-0 left-0 w-full h-0.5 bg-brandgreen transition-transform duration-200 transform scale-x-0 group-hover:scale-x-100 {{ request()->routeIs('contacto') ? 'scale-x-100' : '' }}"></span>
                </a>
